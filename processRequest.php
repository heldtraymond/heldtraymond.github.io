...
if (isset($_POST['a'])) {
	...
	if ($gameAction >= 122400000 && $gameAction < 122416384) {
		...
		$newGameSql = "INSERT INTO GameStatus(Id, Round, RoundStartUtc, CurrentSeat, EndingSeat, UpCardIndex, RoundStartSeat) VALUES (1 + SELECT MAX(Id) FROM GameStatus, 0, SELECT DATEADD(minute, 2, GETUTCDATE()), $startingSeat, NULL, $startingUpCardIndex, $startingSeat);DECLARE @GameId int = (SELECT MAX(Id) FROM GameStatus)";
		...
		$gameStatusSql = "SELECT TOP 1 g.*, s.PlayerId AS PlayerSeat, DATEDIFF(GETUTCDATE(), RoundStartUtc) AS TimeDiff,
			...
			if (!$isDrawTwo && $action != 6) {
				...
				if ($hasDownCard) {
					...
					if (is_null($endingSeat)) {
						...
						if ($round < 9) {
							$round++;
							$updateSql = $updateSql . "UPDATE GameStatus SET Round = $round, RoundStartUtc = DATEADD(second, 160, GETUTCDATE()), RoundStartSeat = $nextRoundStartSeat, CurrentSeat = $nextRoundStartSeat, EndingSeat = NULL WHERE GameId = $gameId;";
						}
					}
				}
			}
...
// Get game status
$gameStatusResult = $conn->query("SELECT TOP 1 *, DATEDIFF(second, GETUTCDATE(), RoundStartUtc) AS TimeLeft FROM GameStatus ORDER BY Id DESC");
$flippedUpCardsResult = $conn->query("SELECT TOP 1 *, DATEDIFF(second, GETUTCDATE(), RoundStartUtc) AS TimeLeft FROM GameStatus ORDER BY Id DESC");
...
$playerCardsResult = $conn->query("SELECT * FROM Card c
							 ...
							 WHERE g.Id = (SELECT MAX(Id) FROM GameStatus)
							 AND c.Round = (CASE WHEN g.NextRoundStartSeat IS NULL THEN g.Round ELSE (g.Round - 1) END)
...
if (count($forceFlipCardIndices) > 0) {
	...
	$forceFlipDrawTwoResult = $conn->query("UPDATE Card SET Value = abs(Value) WHERE GameId = $game AND Round = $round AND DeckIndex IN ($cardIndicesText);");
}
